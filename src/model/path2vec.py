...
class path2vec(object):
    ...
    def score_nodes(self, model_embeddings, index2node, estimate_prob, top_k, display_params: bool = True):
        if display_params:
            self.__print_arguments()
            time.sleep(2)
        X = [n for n in index2node]
        top_nodes = self.predict_nodes(X=X, model_embeddings=model_embeddings, index2node=index2node,
                                       estimate_prob=estimate_prob, top_k=top_k, display_params=False)
        auc(X, top_nodes)

    def __predict_prob_nodes(self, model_embeddings, display_params: bool = True):
        if display_params:
            self.__print_arguments()
            time.sleep(2)
        similarity = np.dot(model_embeddings, model_embeddings.T)
        numerator = np.exp(similarity)
        denominator = np.sum(numerator, axis=1)
        denominator = denominator.reshape((denominator.shape[0], 1))
        denominator[denominator == 0] = EPSILON
        prob_values = numerator / denominator
        return prob_values

    def predict_nodes(self, X, model_embeddings, index2node, estimate_prob, top_k,
                      display_params: bool = True):
        if display_params:
            self.__print_arguments()
            time.sleep(2)
        prob_values = self.__predict_prob_nodes(model_embeddings=model_embeddings, display_params=False)
        if not estimate_prob:
            top_indices = np.argsort(-prob_values, axis=1)[:, 1:top_k + 1]
            top_nodes = np.empty(shape=(X.shape[0], top_k))
            for idx, k in enumerate(X):
                top_nodes[idx] = [index2node[n] for n in top_indices[int(k)]]
        else:
            prob_values = -np.sort(-prob_values, axis=1)[:, 1:top_k + 1]
            top_nodes = prob_values
        return top_nodes
